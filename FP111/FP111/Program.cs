namespace Menú_Principal__Código_final_
{
    internal class Program
    {
        ...
        {
            static string[,] IngresarClientes(string[,] infoGestionClientes)
            {
                //tres columnas - una representando los telefonos, los nombres y los clientes
                ...
                for (int i = 0; i < infoGestionClientes.GetLength(0); i++)
                {
                    ...
                    Console.WriteLine("Para visualizar la lista presione 3, para salir presione otra tecla");
                    ...
                    while (continuar != 1 || continuar != 2 || continuar != 3)
                    {
                        switch (continuar)
                        {
                            case 1 when i == infoGestionClientes.GetLength(0) - 1:
                                //Le avisará al cliente que ya ha llegado a su máximo de clientes
                                Console.WriteLine("Llegó al número máximo de clientes");
                                Console.WriteLine("¿Desea visualizar la lista? Para verla presione 3. Para regresar al menú presione 2");
                                continuar = Int32.Parse(Console.ReadLine());
        ...
        static string[,] ModificarCliente(string[,] infoGestionClientes)
        {
                            ...
                            while (ColumnaParaCambiar == 'c' || ColumnaParaCambiar == 'n' || ColumnaParaCambiar == 't')
                            {
                                switch (ColumnaParaCambiar)
                                {
                                    case 'n': //Caso de que quiera modificar el nombre
                                        ...
                                        if (mostrarListaClientes == 2)
                                        {
                                            ListaClientes(infoGestionClientes);
                                            return infoGestionClientes;
                                        }
                                        else
                                        {
            ...
            static void ListaClientes(string[,] infoGestionClientes)
            {
                int MostrarListaClientes = 0;
                for (int i = 0; i < infoGestionClientes.GetLength(0); i++)
                {
            ...
            //HASTA ACÁ ES LA PARTE DE CLIENTES
